 ...
 */
interface Helper_HtmlParserHandler
{
	/**
...
class Helper_HtmlParser
{
	 ...
	 */
	public function parse($html, Helper_HtmlParserHandler $handler)
	{
		$this->input = $html;
		$this->pos = 0;
	...
	private function commentBlock()
	{
		...
		while ( $this->look( ) !== '>' && $this->pos < $this->len )
		{
			...
			{
				$this->matchAny( );
				// Ignore characters outside comment
			}
		}
		$this->match( '>' );
	...
	{
		return isset( $this->input[$this->pos] ) ? $this->input[$this->pos] : null;
	}
...
class Helper_HtmlFilter implements Helper_HtmlParserHandler
{
	/**
	 * Allowed tags
	 */
	...
	public function openTag($tagName, $attributes)
	{
		...
		if ( isset( $this->allowedAttributes['*'] ) && is_array( $this->allowedAttributes['*'] ) )
		{
			$allowedAttributes = array_merge( $allowedAttributes, $this->allowedAttributes['*'] );
		}

		if ( isset( $this->allowedAttributes[$tagName] ) && is_array( $this->allowedAttributes[$tagName] ) )
		{
			$allowedAttributes = array_merge( $allowedAttributes, $this->allowedAttributes[$tagName] );
		}
		...
		foreach ( $attributes as $name => $value )
		{
			if ( in_array( $name, $allowedAttributes ) )
			{
				...
				if ( in_array( $name, $this->urlAttributes ) )
				{
					...
					if ( (isset( $urlComponents['scheme'] ) && in_array( $urlComponents['scheme'], $this->allowedProtocols )) || (!isset( $urlComponents['scheme'] ) && $this->urlPathAllowed) )
					{
	...
	{
		$this->parser = new Helper_HtmlParser;
	}
